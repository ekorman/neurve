...
class BalancedClassBatchSampler(Sampler):
    def __init__(self, data_source, n_classes, n_per_class):
        ...
        self.labels_to_idxs = {
            int(label): np.where(np.array(data_source.targets) == label)[0].tolist()
    ...
    def __iter__(self):
        ...
        for idxs in self.labels_to_idxs.values():
            random.shuffle(idxs)
        while count < len(self.data_source) / (self.n_classes * self.n_per_class):
            count += 1
            ...
            for cls in classes:
                ...
                if end_idx >= len(cls_idxs):
                    cls_idxs[cls].extend(cls_idxs[: len(cls_idxs) - end_idx + 1])
                indices.extend(cls_idxs[start_idx:end_idx])
            yield indices

    def __len__(self):
        return math.ceil(len(self.data_source) / (self.n_classes * self.n_per_class))
